...
namespace lfp::utils::color_utils {
...
template <typename T>
inline std::string reset(T const& arg) {
    return fmt::format("{}{}§r", ColorFormat::RESET, arg);
}
template <typename T>
...
inline std::string convertToConsole(std::string_view str) {
    return ll::string_utils::replaceMcToAnsiCode(str);
}
